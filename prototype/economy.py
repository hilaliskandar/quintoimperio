"""Protótipo econômico relativo do Quinto Império.

Este módulo lê a evidência estruturada em ``data/`` e parâmetros de
balanceamento em ``simulation/``. Os índices calculados aqui NÃO são preços,
fretes, alíquotas ou probabilidades históricas.

O objetivo da v0.1 é testar relações ordinais e invariantes históricas antes da
escolha do motor gráfico.
"""
...
class EconomyModel:
    """Data-driven ordinal simulation model.

    Historical tables determine *where* a good is produced, imported, exported
    or transited. Simulation tables determine how those roles are translated
    into relative supply, demand, volatility and costs.
    """
    ...
    def __init__(self, root: Path | None = None) -> None:
        ...
        self.nodes = {
            row["node_id"]: row
            for row in _read_csv(self.root / "data" / "nodes.csv")
        }
        self.node_goods = _read_csv(self.root / "data" / "node_goods.csv")
        self.routes = {
            row["route_id"]: row
            for row in _read_csv(self.root / "data" / "routes.csv")
        }
        self.route_goods = _read_csv(self.root / "data" / "route_goods.csv")
        self.goods = {
            row["good_id"]: row
            for row in _read_csv(self.root / "simulation" / "goods_params.csv")
        }

        self.rules: dict[str, dict[str, dict[str, str]]] = {}
        for row in _read_csv(self.root / "simulation" / "rules.csv"):
    ...
    def node_good(
        ...
        if not candidates:
            return None
        # A base atual não deveria ter sobreposição temporal para a mesma chave.
        # Em caso de sobreposição futura, a linha de início mais recente prevalece.
    ...
    def market_quote(
        ...
        O valor ``price_index`` serve apenas para comparar cenários dentro do
        protótipo. Não possui unidade monetária histórica.
        """
        row = self.node_good(node_id, good_id, year)
        if row is None:
            return None
    ...
    def route_cost(
        ...
        intermediation = 0.50 if destination["access_regime"] == "FOREIGN_NEGOTIATED" else 0.15
