...
REASON_LABELS = {
    "MARKET_ACCESS_NOT_GRANTED": "acesso ao mercado ainda não concedido",
...
class M3PlayablePrototype(PlayablePrototype):
    ...
    def adjust_trade_quantity(self, delta: float) -> None:
        self.trade_quantity = min(20.0, max(1.0, self.trade_quantity + delta))
    ...
    def _draw_trade_overlay(
        ...
        pygame.draw.rect(
            surface,
            BUTTON if self.trade_quantity > 1.0 else BUTTON_DISABLED,
            minus,
            border_radius=3,
        )
        pygame.draw.rect(surface, BUTTON, plus, border_radius=3)
