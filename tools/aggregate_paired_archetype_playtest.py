...
def main():
    ...
    summary={'wave':wave,'design':'paired-seed','seeds':sorted(by_seed),'n_sessions':len(rows),'completion_rate':sum(r['completed'] for r in rows)/len(rows),'completed':sum(r['completed'] for r in rows),'top_blockers':blockers.most_common(),'archetypes':{},'seeds_summary':paired}
    for n,it in sorted(by_arch.items()):
        ib=Counter(); [ib.update(r.get('blockers',{})) for r in it]
        summary['archetypes'][n]={'n':20,'completed':sum(r['completed'] for r in it),'completion_rate':sum(r['completed'] for r in it)/20,'secured_reserve_days':it[0].get('secured_reserve_days',0.0),'median_capital_final':med([r['capital_final'] for r in it]),'median_blocked':med([r['blocked_attempts'] for r in it]),'median_min_provisions':med([r['min_provisions'] for r in it]),'median_min_condition':med([r['min_condition'] for r in it]),'top_blockers':ib.most_common()}
