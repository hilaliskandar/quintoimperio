#!/usr/bin/env python3
"""Onda 6: sensibilidade da margem logística antes de toda partida elegível."""
...
def prepare(model,state,m,seed,reserve,c):
    for _ in range(8):
        m.attempt(); m.readiness_checks+=1; plan=model.plan_current_leg(state,seed=seed); m.executed()
        blockers=[r for r in plan.blockers if r in RESOURCE_BLOCKERS or 'PROVISION' in r]
...
def run(pid,profile,seed,reserve):
    model=HistoricalCampaignModel(); pm=CampaignProgressModel(model.session); state=model.initial_state(active_expedition_id='EXP_GAMA_1497'); m=Metrics(pid,profile,seed,6); c={'reserve_actions':0,'reserve_days_added':0.0,'reserve_failed':0,'reserve_skipped':0}; m.observe(state); start=state.vessel.clock.current_date
    while model.current_leg(state) is not None:
        state=proactive_policy(model,state,m,profile)
        state=prepare(model,state,m,seed,reserve,c)
