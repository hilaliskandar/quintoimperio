...
def main():
 ...
 s={'wave':12,'n_archetypes':10,'sessions_per_archetype':20,'n_sessions':len(rows),'completed':sum(r['completed'] for r in rows),'completion_rate':sum(r['completed'] for r in rows)/len(rows),'counterfactual':sum(r['counterfactual'] for r in rows),'positive_provision_events':sum(r['positive_provision_events'] for r in rows),'negative_provision_events':sum(r['negative_provision_events'] for r in rows),'timing_events':sum(r['timing_events'] for r in rows),'top_blockers':blockers.most_common(),'archetypes':{}}
