...
def run_player(player_id,archetype,seed):
    policy=ARCHETYPES[archetype]; model=HistoricalCampaignModel(); progress_model=CampaignProgressModel(model.session); state=model.initial_playable_state(); metrics=Metrics(player_id=player_id,profile=archetype,seed=seed,wave=13); metrics.observe(state); start=state.vessel.clock.current_date
